...
export const publicApi = {
  ...
  registerClick: (id: number) => {
    // 访问统计只记录管理员自己的访问；游客点击不应产生统计请求。
    if (!getAuthToken()) return Promise.resolve(null)
    return request<null>(`/public/bookmarks/${id}/click`, { method: 'POST', auth: true, keepalive: true })
  },
}
...
export const bookmarksApi = {
  ...
  reorganize: (category_orders: BookmarkReorganizeReq['category_orders'], all_orders: BookmarkReorganizeReq['all_orders'] = []) =>
    jsonRequest<null>('/bookmarks/reorganize', 'POST', { category_orders, all_orders }, true),
