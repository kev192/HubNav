...
<fieldset id="settings-section-network" class="group group-wide" disabled={saving}>
  ...
  <div class="form-grid network-grid">
    <label class="field network-mode-field">
      <span>默认网络模式</span>
      <select bind:value={form.network_mode}>
        <option value="external">外网</option>
        <option value="internal">内网</option>
        <option value="auto">自动检测</option>
      </select>
      <small>访客打开书签时使用的默认地址类型。</small>
    </label>
    <label class="field network-probe-field">
      <span>自动模式判别 URL</span>
      <input bind:value={form.network_probe_url} type="url" placeholder="https://example.com/health" />
      <small>仅在选择“自动检测”时使用，用于判断当前网络是否可访问内网。</small>
    </label>
    <div class="probe-row">
...
<style>
  ...
  .network-grid .field input,
  .network-grid .field select {
    min-height: 42px;
  }

  .network-grid .field small {
    min-height: 2.8em;
    font-size: 12px;
  }
