...
describe('home responsive layout', () => {
  ...
  it('compacts the mobile top action row and category bar without changing desktop layout', () => {
    ...
    expect(sidebar).toContain('<div class="mobile-top-mask" class:visible={topNavScrolled} aria-hidden="true"></div>')
    expect(mobileSidebar).toContain('.top-navigation.scrolled::before {\n      display: none;')
    expect(mobileSidebar).toContain('.mobile-top-mask {')
    expect(mobileSidebar).toContain('position: fixed;')
    expect(mobileSidebar).toContain('top: 0;')
    expect(mobileSidebar).toContain('height: calc(.65rem + 2.56rem);')
    expect(mobileSidebar).toContain('left: 0;')
    expect(mobileSidebar).toContain('right: 0;')
    expect(mobileSidebar).toContain('z-index: 58;')
