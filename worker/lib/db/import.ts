...
export async function importData(
  ...
  for (const chunk of chunkImportRows(importedBookmarks, 7)) {
    stmts.push(db.prepare(`INSERT INTO bookmarks (id, category_id, title, url, icon, icon_source, icon_background_color, icon_blob, description, description_mode, open_method, is_private, sort, all_sort, created_at) VALUES ${chunk.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').join(', ')}`)
