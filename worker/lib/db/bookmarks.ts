   ...
   .prepare(
    `INSERT INTO bookmarks (
         ...
         WHERE EXISTS (SELECT 1 FROM categories WHERE id = ?)
           RETURNING id, category_id, title, url, internal_url, icon, icon_source, icon_background_color, icon_blob, description, description_mode, open_method, is_private, sort, all_sort, click_count, created_at`,
   )
   .bind(
    req.category_id,
    req.title,
